<div class="relative | bg-white dark:bg-dt-dark focus:outline-none">
    ...
    <div class="p-6 | flex flex-col items-center">
        <p>¿Estás seguro que quieres aceptar la solicitud?</p>
        <p class="mt-2.5 text-sm"><strong>{{ $eteamRequest->user->name }}</strong> se unirá al equipo <strong>{{ $eteamRequest->eteam->name }}</strong>.</p>
    </div>

    {{--footer--}}
    <div class="p-4 border-t border-border-color dark:border-edgray-700 | flex items-center justify-end">
        <form method="GET" action="{{ route('my-teams.acceptRequest', $eteamRequest) }}">
            @csrf
            ...
            <button type="button" wire:click="$emit('closeModal')" class="w-24 inline-block px-4 py-1.5 bg-rose-600 text-white text-xxs leading-tight rounded shadow-md hover:bg-rose-700 hover:shadow-lg focus:bg-rose-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-rose-800 active:shadow-lg transition duration-150 ease-in-out">
                Cancelar
            </button>
        </form>
    </div>
