<div class="relative | bg-white dark:bg-dt-dark focus:outline-none">
    ...
    <div class="p-6 | flex flex-col items-center">
        ...
        <p class="mt-2.5 text-sm"><strong>{{ $eteamRequest->user->name }}</strong> se unirá a tu equipo "<strong>{{ $eteamRequest->eteam->name }}</strong>".</p>
    </div>

    {{--footer--}}
    <div class="p-4 border-t border-border-color dark:border-edgray-700 | flex items-center justify-end">
        <form method="GET" action="{{ route('my-teams.acceptRequest', [$user->id, $eteamRequest->id]) }}">
            @csrf
