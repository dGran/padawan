    ...
    @if ($someFilterApplied)
        <ul class="mt-4 flex items-center space-x-4">
            ...
            @if (!empty($contextFilter))
                <li>
                    @include('eteam.admin.logs._filter-tag', ['title' => 'Contexto', 'value' => $contextFilter, 'filterName' => 'contextFilter'])
                </li>
            @endif

            @if (!empty($typeFilter))
                <li>
                    @include('eteam.admin.logs._filter-tag', ['title' => 'Tipo', 'value' => $typeFilter, 'filterName' => 'typeFilter'])
                </li>
            @endif
        </ul>
    @endif
