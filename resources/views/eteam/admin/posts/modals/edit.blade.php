    ...
    <form wire:submit.prevent="update">
        ...
        <div class="p-4 flex flex-col space-y-4">
            <div>
                ...
                <x-input wire:model="eteamPost.title" id="title" class="text-sm mt-1.5 w-full" type="text" placeholder="Escribe el título de la noticia"></x-input>
            </div>
            <div>
        ...
        <div class="p-4 border-t border-border-color dark:border-edgray-700 | flex items-center justify-end">
            ...
            <x-button type="submit" class="{{ empty($eteamPost->title) || empty($eteamPost->content) ? 'pointer-events-none opacity-50' : '' }} inline-block px-4 py-1.5 bg-edblue-600 text-white text-xxs leading-tight rounded shadow-md hover:bg-edblue-700 hover:shadow-lg focus:bg-edblue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-edblue-800 active:shadow-lg transition duration-150 ease-in-out" @click="open = false">
                {{ __('Guardar cambios') }}
            </x-button>
        </div>
    </form>
