<tr data-id="{{ $reserve->id }}">
    <td class="select text-center">
        <label class="custom-check">
            <div>
                <input type="checkbox" class="mark" value="{{ $reserve->id }}" name="reserveId[]" onchange="showHideRowOptions(this)"/>
        ...
        <div class="flex items-center">
            <div class="flex-shrink-0 w-10 h-10">
                <img class="object-cover w-full h-full rounded"
                    src="{{ $reserve->presenter()['img'] }}"
                    alt="" />
            </div>
            <div class="ml-3">
                {{-- <p class="text-gray-900 whitespace-no-wrap"> --}}
                    <span class="name">{{ $reserve->presenter()['name'] }}</span>
                    <span class="block text-gray-600" style="font-size: 9px">ID:{{ $reserve->id }}</span>
                {{-- </p> --}}
            </div>
        </div>
    </td>
