    ...
    <div class="form">
        <form id="form-edit" method="POST" role="form" action="{{ route('admin.circuits.update', $circuit->id) }}" lang="{{ app()->getLocale() }}" enctype="multipart/form-data">
			...
			<div class="flex flex-row mb-3 rounded justify-center">
				<div class="relative">
					<img id="thumbnail" src="{{ $circuit->img() }}" alt="img" class="thumbnail original-size">
            ...
            <div class="field-group">
                <div class="element">
                    ...
                    <input type="number" class="placeholder-gray-400" id="length" name="length" placeholder="Longitud (metros)" value="{{ old('length', $circuit->length) }}">
                </div>
                <div class="element">
                    ...
                    <div class="relative">
                        <select name="country_id" id="country_id">
                            ...
                            @foreach ($countries as $country)
                                <option {{ $circuit->country_id == $country->id ? 'selected' : '' }} value="{{ $country->id }}">
                                    {{ $country->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="mt-8">
                ...
                <a href="{{ route('admin.circuits') }}" class="bg-transparent text-red-500 active:text-red-600 focus:text-red-600 hover:text-red-600 font-bold uppercase text-sm px-4 py-3 rounded outline-none focus:outline-none ml-2" style="transition: all .15s ease">
                    Cancelar
                </a>
            </div>
