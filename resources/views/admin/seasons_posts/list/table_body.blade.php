<tr data-id="{{ $season_post->id }}">
    <td class="select text-center">
        <label class="custom-check">
            <div>
                <input type="checkbox" class="mark" value="{{ $season_post->id }}" name="season_postId[]" onchange="showHideRowOptions(this)"/>
        ...
        <div class="flex items-center">
            ...
            <div class="ml-3">
                {{-- <p class="text-gray-900 whitespace-no-wrap"> --}}
                    <span class="name">{{ $season_post->title }}</span>
                    <span class="block text-gray-600" style="font-size: 9px">ID:{{ $season_post->id }}</span>
                {{-- </p> --}}
            </div>
        </div>
    </td>
    <td onclick="rowSelect(this)">
        <span class="name">{{ $season_post->type_name() }}</span>
    </td>
</tr>