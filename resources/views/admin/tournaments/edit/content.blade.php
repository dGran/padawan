<div class="antialiased font-sans flex px-4 md:px-8 pb-2">
    ...
    <div class="form">
        <form id="form-edit" method="POST" role="form" action="{{ route('admin.tournaments.update', $tournament->id) }}" lang="{{ app()->getLocale() }}" enctype="multipart/form-data">
            ...
            <input type="file" name="banner" id="banner" value="{{ $tournament->banner }}" onchange="showBanner(this)" style="display:none"/>
            <input type="hidden" name="deleteBanner" id="deleteBanner" value=0>
            <div class="flex flex-row mb-3 rounded justify-center">
                <div class="relative">
                    <img id="thumbnail_banner" src="{{ $tournament->banner() }}" alt="banner" class="thumbnail original-size">
                    <a id="delete_banner" class="{{ is_null($tournament->banner) ? 'hidden' : '' }} absolute rounded-full h-8 w-8 flex items-center justify-center bg-red-500 text-white active:bg-red-600 font-bold outline-none focus:outline-none text-xl cursor-pointer" onclick="deleteBanner()" style="top: -5px; right: -10px">
                        <i class="fas fa-times"></i>
                    </a>
                </div>
            </div>
