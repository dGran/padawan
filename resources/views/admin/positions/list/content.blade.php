<div class="antialiased font-sans w-full px-4 md:px-8">
    <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 pt-4 pb-2">
        @include('admin.positions.list.filters')
        @include('admin.positions.list.filter_tags')
    </div>
        ...
        <div class="table-wrap">
            <table class="admin-tables">
                @if ($positions->count() > 0)
                    <thead>
                        @include('admin.positions.list.table_header')
                    </thead>
                @endif
                <tbody>
                    @if ($positions->count() > 0)
                        @foreach ($positions as $position)
                            @include('admin.positions.list.table_body')
                        @endforeach
                    @else
            ...
            @if ($positions->count() > 0)
                <div class="px-5 py-4 bg-white border-t flex flex-col xs:flex-row items-center xs:justify-between">
                    @include('admin.positions.list.table_footer')
                </div>
            @endif
        </div>
    </div>
...
<div class="selected-options hidden animated fast">
    @include('admin.positions.list.selected_options',
        [
            'edit'        => 1,
            'view'        => 1,
            'destroy'     => 1,
...
<div class="global-options hidden animated fast">
    @include('admin.positions.list.global_options',
        [
            'import'      => 1,
            'export'      => 1
        ])
