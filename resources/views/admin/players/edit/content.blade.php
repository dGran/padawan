        ...
        <form id="form-edit" method="POST" role="form" action="{{ route('admin.players.update', $player->id) }}" lang="{{ app()->getLocale() }}" enctype="multipart/form-data">
                    ...
                    <div class="relative">
                        <select name="players_databases_id" id="players_databases_id">
                            @foreach ($players_databases as $player_database)
                                <option {{ $player->player_database_id == $player_database->id ? 'selected' : '' }} value="{{ $player_database->id }}">
                ...
                <div class="element">
                    ...
                    <div class="relative">
                        <select name="position_id" id="position_id">
                            <option {{ $player->position_id == null ? 'selected' : '' }} value="">*Desconocida</option>
                            @foreach ($positions as $position)
                                <option {{ $player->position_id == $position->id ? 'selected' : '' }} value="{{ $position->id }}">
                                    {{ $position->name }} ({{ $position->game->name }} - {{ $position->game->platform->name }})
                                </option>
                            @endforeach
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
            ...
            <div class="field-group">
                <div class="element">
                    ...
                    <input type="number" class="placeholder-gray-400" id="overall_rating" name="overall_rating" placeholder="Media general" value="{{ old('overall_rating', $player->overall_rating) }}">
                </div>
                <div class="element">
                    <label for="height">Altura</label>
                    <input type="number" class="placeholder-gray-400" id="height" name="height" placeholder="Altura" value="{{ old('height', $player->height) }}">
                </div>
                <div class="element">
                    <label for="age">Edad</label>
                    <input type="number" class="placeholder-gray-400" id="age" name="age" placeholder="Edad" value="{{ old('age', $player->age) }}">
                </div>
                <div class="element">
            ...
            <div class="field-group">
                ...
                <div class="element">
                    <label for="league_name">Liga</label>
                    <input type="text" class="placeholder-gray-400" id="league_name" name="league_name" placeholder="Liga" autofocus value="{{ old('league_name', $player->league_name) }}">
                </div>
            </div>
