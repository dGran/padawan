@if ($paginator->hasPages())
    ...
    <div class="flex flex-col items-center">
        <div class="flex text-gray-700">
            ...
            @else
                <a class="h-8 w-8 mr-1 flex justify-center items-center rounded-full bg-gray-200 hover:bg-pink-600 hover:text-white cursor-pointer transition duration-150 ease-in" href="{{ $paginator->previousPageUrl() }}" rel="previous">
                    <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-chevron-left w-4 h-4">
                        <polyline points="15 18 9 12 15 6"></polyline>
                    </svg>
                </a>
...
{{--                 @if($paginator->currentPage() > 3)
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            @else
                                <a class="w-8 md:flex justify-center items-center hidden cursor-pointer leading-5 transition duration-150 ease-in rounded-full hover:bg-pink-600 hover:text-white" href="{{ $url }}">{{ $page }}</a>
                            @endif
                        @endforeach
                    @endif
                @endforeach
            ...
            @if ($paginator->hasMorePages())
                <a class="h-8 w-8 ml-1 flex justify-center items-center rounded-full bg-gray-200 cursor-pointer hover:bg-pink-600 hover:text-white transition duration-150 ease-in" href="{{ $paginator->nextPageUrl() }}" rel="next">
