<nav x-data="{ open: false }">
        ...
        <div class="flex justify-between items-center h-16">
            <div class="flex">
                ...
                <div class="flex items-center md:hidden">
                    <button @click="open = ! open"
                            class="inline-flex items-center justify-center | font-medium | hover:text-title-color dark:hover:text-dt-title-color | focus:outline-none focus:text-title-color dark:focus:text-dt-title-color | transition duration-150 ease-in-out">
                ...
                <div class="flex-shrink-0 flex items-center ml-2 md:ml-0">
                    <x-link href="{{ route('dashboard') }}"
                            class="group | flex items-center | focus:no-underline hover:no-underline">
                        {{-- <i class="icon-logo | text-3xl md:text-4xl"></i> --}}
                        <img src="{{ asset('img/logo2.png') }}" alt=""
                             class="w-auto h-10 opacity-100 | group-hover:opacity-80 | group-focus:opacity-80">
                        <p class="whitespace-nowrap font-ubuntuCondensed tracking-tight text-xl | ml-0">
                            @php
                                $appName = explode(' ', config('app.name'));
                            @endphp
                            <span
                                class="text-title-color dark:text-dt-title-color | group-hover:text-black dark:group-hover:text-white | group-focus:text-black dark:group-focus:text-white">
                                {{ $appName[0] }}
                            </span>
                            <span
                                class="text-edblue-500 dark:text-edblue-400 | group-hover:text-edblue-600 dark:group-hover:text-edblue-300 | group-focus:text-edblue-600 dark:group-focus:text-edblue-300">
                                {{ $appName[1] }}
                            </span>
                        </p>
                    </x-link>
                ...
                <ul class="hidden space-x-4 md:-my-px md:ml-8 md:flex">
                    ...
                    @foreach ($navLayoutMenuLinks as $link)
                        <x-nav-link :href="route($link['route-name'])">
                            <span class="{{ $link['class'] }}">{{ __($link['text']) }}</span>
                        </x-nav-link>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="flex items-center">
                ...
                <button id="theme-toggle" type="button"
                        class="uppercase inline-flex items-center px-1 pt-1 mr-3 text-xl leading-5 hover:text-title-color dark:hover:text-dt-title-color | focus:outline-none focus:text-title-color dark:focus:text-dt-title-color focus:outline-none transition duration-150 ease-in-out">
    ...
    <div x-cloak class="md:hidden absolute w-full h-screen bg-gray-50 dark:bg-dt-darker z-40 shadow-md"
         id="responsiveNavMenu" x-show="open"
        ...
        <ul class="bg-white dark:bg-dt-dark | border-b border-border-color dark:border-dt-border-color | w-full"
