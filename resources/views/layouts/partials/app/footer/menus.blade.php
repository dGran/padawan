		        ...
		        <div x-show="selected == 1" class="border-b border-gray-300 dark:border-dt-darker px-6 py-1.5"
		            ...
		            <ul class="flex flex-col | py-3">
                        @foreach($navLayoutMenuLinks as $link)
		                    <li class="py-1"><a href="{{ route($link['route-name']) }}" class="block | hover:text-title-color dark:hover:text-dt-text-color | focus:outline-none focus:text-title-color dark:focus:text-dt-title-color | transition duration-150 ease-in-out">{{ $link['text'] }}</a></li>
                        @endforeach
		            </ul>
		        </div>
		    </li>
	...
	<x-container class="hidden md:block">
		<div class="flex items-start justify-between gap-20 select-none | text-xs">
			<div class="flex-initial">
		        ...
		        <ul class="flex flex-col | list-none | ml-1.5 pl-3 border-l border-edgray-400 dark:border-edgray-800">
                    @foreach($navLayoutMenuLinks as $link)
